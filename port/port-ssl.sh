...
CEKEXPIRED () {
    ...
    Exp1=$(curl -sS https://raw.githubusercontent.com/envy26/maem/main/ip | grep $MYIP | awk '{print $3}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl -sS https://raw.githubusercontent.com/envy26/maem/main/ip | awk '{print $4}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
